...
[English](2026-09-12-cli-fix-report.md) | 中文
...
## 2026-09-13 /sessions 与 /new 实机问题修复

### 本次新增问题
1. 从 `/sessions` 打开历史会话后执行 `/new`，旧会话内容仍残留在新会话界面上。
2. 连续执行 `/new` 会产生大量空会话，`/sessions` 列表包含用户从未输入过内容的 Session。
3. Session 列表按创建时间排序，而不是按最后一次用户输入时间排序。

### 修复方案
- `/new` 改为真正触发 Session lifecycle 的 `ExitRequest({ type: 'new' })`；旧 Session dispose 后由宿主重新创建 fresh Session，并清理旧 viewport，避免旧 Static/transcript 残留。
- 当前 Session 如果从未出现用户输入，则 `/new` 不创建新 Session，而是复用当前空 Session。
- `/sessions` 列表读取 `sessionListMetadata` projection：只有 `blank === false` 的 Session 才进入列表。也就是必须存在已经提交的用户消息，纯 `/new` 创建的空 Session 不显示。
- 列表新增 `updatedAt`，使用 `lastPromptAt` 作为活动时间；列表按 `updatedAt` 倒序排列。
- Session picker 的时间显示同步改用 `updatedAt`。

### 验证
- cli-app 关键 TS/TSX 文件使用仓库内 esbuild 0.28.1 分别进行 transform 检查，当前结果为 0 个语法失败。
- `git diff --check` 未发现 whitespace 错误。
- `pnpm exec tsc --noEmit` 在当前工具执行环境无法真正启动：该环境的 `pnpm.ps1` 找不到 `node.exe`，因此不能把 TypeScript 全量检查宣称为通过。
