...
`aicode` —— deepseek dsh 终端 AI 编程助手的 npm 全局安装封装。

安装后获得全局命令 `aicode`，打开 code profile 的 Ink 终端交互界面（TUI），
...
> 命令名刻意使用 `aicode`（而非 `code`），避免与 VS Code 自带的 `code`
> 命令冲突；两者可共存互不影响。0.1.2 起原 `code` 命令更名为 `aicode`。
...
- `aicode` 之后的参数原样透传给 dsh 启动器，例如
  `aicode --dump-config`、`aicode --plugin <name>`。
...
| 部分 | 来源 |
| --- | --- |
| `bin/aicode.js` | 启动器：释放内置前端、初始化 profile，并拉起 `dsh --profile code` |
| `@deepseek-ai/dsh` | 官方 dsh 启动器（公开 npm 包，锁定 `0.1.3-alpha.2`） |
| `vendor/dsh-cli-app/` | 本仓库内置的 code TUI 前端包（Ink），首次启动 `aicode` 时自动释放到包内 node_modules |
