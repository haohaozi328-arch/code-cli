...
- Registry slash commands completed: send() routes non-builtin `/xxx` lines to `ctx.commands.execute` (the base command-* rows: /compact /feedback /goal /plan; executing them consumes no model turn); unregistered names get an unknown hint (no longer sent to the model); HELP updated. (Revised 2026-09-20: a non-registry slash line may instead route to a registered user-invocable skill as a verbatim prompt before falling back to the hint — see `/skills invocation`.)
...
The user asked for a standalone **task progress-time board** beyond the todo panel: `Ctrl+B` swaps the whole screen between the conversation and the board (while up, the board swallows every other key). The board reads only state the view model already projects, so it can never disagree with the transcript: the task checklist (the `todo/write` projection, full `[✓]/[•]/[ ]` rows plus a done count), the current turn (spinner + live elapsed + queue depth while running; last turn duration and its end clock when idle), a per-turn conversation timeline (folded from `turn/start`/`user/message`/`assistant/message`/`tool/call`/`turn/end`: each turn's clock span plus the human prompt, the assistant reply summary, the tool sequence, and output tokens, `#turn HH:MM → HH:MM duration`, newest first, capped at 12 with ring discard, rebuilt from the durable log on resume), the model/permission/context-ring/usage readouts, and a pending-approval warning (the board must not hide an unanswered tool gate). Implementation: `ui/taskboard.tsx` owns `reduceTurnSpans/collectTurnSpans` (pure, reference-stable folds) and `isBoardToggle` (the Ctrl+B test over Ink's ctrl+meta key flags); the VM gains `boardOpen/turnSpans` state and a `toggleBoard` action (replay folds the history from the durable log once, the live path consumes `session/event`); App handles the toggle before every other key branch. The first cut used Ctrl+Alt and never fired on the real machine (Windows Terminal encodes Alt+letter as `ESC letter`; ink's keypress parser never sets `meta` for that shape), so the chord became Ctrl+B — a plain control character every terminal encodes reliably. The same round upgraded the session-boundary clear to a full wipe including the scrollback (`ESC[3J`): after `/new` and friends the retired conversation can no longer be scrolled back into view, while `/clear` keeps the scrollback. `/help` gained the chord line. Convention: tests never import `.tsx` directly (the host aggregate's TS6305 trap) — everything goes through the `ui/index.ts` barrel. Same-day second iteration (the user called the first cut too plain): the timeline became a conversation timeline — `TurnEntry` (prompt/reply/tools/outputTokens) replaced the duration-only `TurnSpan`, the fold consumes user/assistant/tool events (injected messages and empty text produce no entry, tools dedupe, the prompt keeps the latest), and the VM field `turnSpans` became `turnTimeline`. Gates: vitest 122/122, oxlint 0, tsc 0, whole-repo typecheck green, both profiles boot-smoked clean. Same-day third iteration (the user asked to drag the timeline): the board became a two-pane scrubber — the timeline renders as a cursor axis (`●` marks the live turn, `❯` the selected one) and a content pane shows the selected turn's full folded conversation (`TurnEntry` gained `messages: TurnMessage[]` user/assistant/tool rows, each message capped at 8 rendered lines with a more-marker, retention capped at 40 per turn); ↑/↓ step one turn, PgUp/PgDn page five, stepping past the newest returns to live. `stepTimelineCursor` is a pure cursor function; ink's `Key` type has no home/end flags, so only the arrows and page keys are bound. Gates: vitest 125/125, oxlint 0, tsc 0, whole-repo typecheck green, test:docs 15/15, both profiles boot-smoked clean (two unrelated suites flaked only under full-workspace parallel load and pass in isolation). Before release the user's machine hit an idle OOM (4 GB heap in about three minutes) that traced to ink's static handling: closing the board unmounted the transcript `<Static>`, the remount re-rendered the whole transcript as fresh static output that ink retains and replays whenever a frame reaches screen height, and the write flood backed up the terminal pipe onto the heap. The Static element is now the first child of every App branch so it stays mounted, and the board clamps itself below the terminal height (axis rows and pane messages budgeted, pane lines width-truncated to one physical row). A probe harness measured the write flood dropping from 4.3 GB to 2 MB per 800 board toggles and from 3.7 GB to 3 MB per 15 seconds of spinner. Gates: vitest 126/126, oxlint 0, tsc 0, whole-repo typecheck green, test:docs 15/15, both profiles boot-smoked clean. Same-session follow-up (the user picked paging, history, and titles): the board's content pane now pages — `packPanePages` packs a turn into pane-budget pages newest-first, the left/right arrows walk them (a page counter and a later-messages marker render in the pane), and switching turns lands on the newest page. The composer recalls sent prompts with the up/down arrows (`ui/history.ts`: one process-wide store, seeded per session from its durable prompts, capped at 100, with the in-flight draft preserved for the return to live), and `/title <text>` renames through the official `ctx.sessionTitle.rename` seam — the VM re-derives its status-bar label from the durable `session/title` event (provider titles refresh it too), cli-app gained a project reference on session-title, and a rename pins the title against automatic retitling; bare `/title` then became an inline editor modeled on the /connect wizard: `UiState.titleEditor` carries the current durable title, the shared composer buffer feeds it, Enter submits through `ctx.sessionTitle.rename` and Esc cancels; `/title <text>` still renames inline. Gates: vitest 139/139, oxlint 0, tsc 0, whole-repo typecheck green, both profiles boot-smoked clean. Follow-up (the user found /feedback useless): the command is gone from the terminal surface — dropped from COMMAND_HINTS and the /help text, `/feedback ...` now answers with the unknown-command hint instead of executing, and the README registry list drops it. The bundle still mounts the plugin package: deleting it outright would ripple through the base-bundle manifest, otel telemetry deps, and the official composition docs, so only the CLI surface was removed.

## /skills invocation (2026-09-20)

The user asked for Claude Code / opencode-style skills in the terminal: `/skills` summons the skill list, a pick lands `/name ` in the composer, the UI waits for guidance, and the completed line invokes the skill.

- **The host keeps invocation authority; the TUI only stages tokens.** `ctx.commands` (the registry cli-app mounts in its patch) and `ctx.skills` (the base registry plus its filesystem provider) stay the two authorities, and a `/name [guidance]` line resolves **command first** (`commands.find`), then **skill**: a user-invocable hit forwards the line verbatim as a user message, because the host's pre-step boundary already recognizes a whitespace-bounded `/name` token in a sent message and injects the rendered `<skill_content>` (`dsh-tool-skill` + `renderSkillContent`) — the same load path as the web client's menu pick, a hand-typed token, or an ACP prompt. Unknown or model-only names keep the unknown-command hint; a line typed mid-run queues like any prompt. Injecting in the TUI was rejected: it would fork the canonical `<skill_content>` shape and the `skill` tool's anti-double-load rule.
- **/skills picker**: a choice list over `skillRegistry.list({cwd, scope: agent})` filtered by `isUserInvocable` (async; load failure degrades to a notice row). Enter on a choice runs `pickSkill` (the VM only closes the picker) and the App branch seeds the composer with `/name `; send-time validation lives entirely in the slash fallback's skill route. `/skills <arg>` is a usage error; zero user-invocable skills posts the unavailable notice instead of opening a picker that would only answer Esc; a missing registry answers with the same notice synchronously, so the registry-less CommandRuntime bench still covers the old unknown-command path. Rows are one-liners after the same-day density pass: `ChoiceItem` gained an optional `description`, rows render `/name` (the invocation shorthand) with the simple description clipped at the terminal edge via `truncate-end` (label column padded, capped at 24), so a long catalog summary can never wrap into a dense wall.
- cli-app's tsconfig gained the `interaction/commands` + `skill/skill` project references; package.json gained the matching workspace deps.
- Same-day reliability pass (the user hit a stuck Esc): the catalog scan is asynchronous, so `/skills` now opens a loading frame synchronously (placeholder note in `ChoicePickerState.note`), and `setChoicePicker` bumps a `choiceEpoch` on every write — a list resolving after the user already left (Esc, or another picker) can no longer reopen a dismissed picker. Esc on the choice list also clears whatever trickled into the composer during the wait.
- Same-day follow-up (per user): every slash-menu hint is Chinese now (`COMMAND_HINTS`), and the choice picker filters as you type — App-owned `choiceSearch` narrows the rows over label and description, Backspace edits the query, the footer echoes `筛选：<query>`, and an empty result reads 无匹配项; the cursor/search reset fires only on a fresh open, so the async catalog swap never drops an in-flight query. Gates: vitest 155/155.
- Gates: vitest 150/150 (new `skill invocation` describe in state.spec + `App skill picker` describe in app.spec), oxlint 0, whole-repo typecheck green, repo-wide lint 0.

## Working-copy merge notes

- 2026-09-20: cli-app's tsconfig additionally gained the `interaction/commands` and `skill/skill` project references (preserve them on merge too).
