...
The user asked for a standalone **task progress-time board** beyond the todo panel: `Ctrl+Alt` swaps the whole screen between the conversation and the board (while up, the board swallows every other key). The board reads only state the view model already projects, so it can never disagree with the transcript: the task checklist (the `todo/write` projection, full `[✓]/[•]/[ ]` rows plus a done count), the current turn (spinner + live elapsed + queue depth while running; last turn duration and its end clock when idle), a per-turn timeline (wall-clock spans folded from `turn/start`/`turn/end`, `#turn HH:MM → HH:MM duration`, newest first, capped at 12 with ring discard), the model/permission/context-ring/usage readouts, and a pending-approval warning (the board must not hide an unanswered tool gate). Implementation: `ui/taskboard.tsx` owns `reduceTurnSpans/collectTurnSpans` (pure, reference-stable folds) and `isBoardToggle` (the Ctrl+Alt test over Ink's ctrl+meta key flags); the VM gains `boardOpen/turnSpans` state and a `toggleBoard` action (replay folds the history from the durable log once, the live path consumes `session/event`); App handles the toggle before every other key branch. `/help` gained the chord line. Convention: tests never import `.tsx` directly (the host aggregate's TS6305 trap) — everything goes through the `ui/index.ts` barrel. Gates: vitest 120/120 (9 new cases), oxlint 0, tsc 0, whole-repo typecheck green, both profiles boot-smoked clean.
