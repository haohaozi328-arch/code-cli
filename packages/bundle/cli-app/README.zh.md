...
- 全屏任务看板（`dsh-taskboard`）：`Ctrl+B` 把对话切换为看板——任务清单、可拖动的逐回合对话时间轴（↑↓ 或 PgUp/PgDn 选择回合，内容窗格显示该回合完整对话：时钟时段、你的提问、助手回复、工具调用与输出 token；`●` 标记 live 回合、`❯` 标记选中回合）、运行中的实时用时、排队深度与用量/上下文读数；同一组合键切回。
