...
- **Input**: single-line input with arrow-key editing. A paste (one chunk of 80+ characters, or any chunk containing a newline) is folded in the prompt as `【head...tail，N字符】` while the buffer keeps the text verbatim. The placeholder is atomic: the caret steps across it in one move and a single Backspace/Delete removes the whole pasted region, never one invisible character at a time. Typing inside a folded region unfolds it, and Enter always sends the full text.
